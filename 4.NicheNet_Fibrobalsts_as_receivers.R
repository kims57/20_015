...
nichenet_output = nichenet_seuratobj_aggregate(
  ...
  sender =  c("Endothelial", "B", "Mononuclear_Phagocyte", "Epithelial", "Neutrophil","T", "Neural", "Smooth_Muscle"), 
...
#Ligand vs DE Features Regulatory Potential Heatmap: 
  pdf(file="~/20_015/nichenet_output2/DE_genes_with_ligands.pdf", width = 7, height = 4.5)
...
pdf(file="~/20_015/nichenet_output2/summary_ligand_activity_target_heatmap_fibroblast_target.pdf", width = 14, height = 8)
nichenet_output$ligand_activity_target_heatmap
dev.off()

pdf(file="~/20_015/nichenet_output2/summary_ligand_activity_target_heatmap_fibroblast_target_tall.pdf", width = 14, height = 9)
nichenet_output$ligand_activity_target_heatmap
dev.off()


#Ligand vs Receptor Heatmap: 
pdf(file="~/20_015/nichenet_output2/fibroblast_receptor.pdf", width = 5.75, height = 6)
...
ligand_type_indication_df = tibble(
  ligand_type = c(rep("Fibroblast_specific", times = Fibroblasts_specific_ligands %>% length()),
                  rep("Endothelial1_specific", times = Endothelial1_specific_ligands %>% length()),
                  ...
                  rep("Epithelial1_specific", times = Epithelial1_specific_ligands %>% length()),
                  rep("Endothelial2_specific", times =Endothelial2_specific_ligands %>% length()),
                  rep("Neutrophil_specific", times = Neutrophil_specific_ligands %>% length()),
                  rep("T_specific", times = T_specific_ligands %>% length()),
                  rep("general", times = general_ligands %>% length())),
  
  ligand = c(Fibroblasts_specific_ligands,
             Endothelial1_specific_ligands,
             B_specific_ligands,
             `Mononuclear-Phagocyte_specific_ligands`,
             Epithelial1_specific_ligands,
             Endothelial2_specific_ligands,
             Neutrophil_specific_ligands,
             T_specific_ligands,
             general_ligands))
...
active_ligand_target_links_df$target_type <- "Gingiva_vs_Perio-Fibroblasts_DE" # needed for joining tables
...
ligand_colors <- c("general" = "green3",
                   "T_specific" = "cyan2",
                   "Neutrophil_specific" = "purple") 

target_colors <- c("Gingiva_vs_Perio-Fibroblasts_DE" = "steelblue2") 

vis_circos_obj <- prepare_circos_visualization(circos_links,
                                               ligand_colors = ligand_colors,
                                               target_colors = target_colors,
                                               celltype_order = c("T_specific",
                                                                  "Neutrophil_specific",
                                                                  "general"))


pdf(file = "~/20_015/nichenet_output2/Circos_Perio_Fibroblasts_Ligands-Targets.pdf", width = 6, height = 6)
...
lr_network_top_df <- nichenet_output$ligand_receptor_df %>%
  mutate(target_type = "Fibroblast_receptors") %>%
  rename(target=receptor) %>%
  inner_join(ligand_type_indication_df)

receptor_colors <- c("Fibroblast_receptors" = "gray")
...
vis_circos_receptor_obj <- prepare_circos_visualization(lr_network_top_df,
                                                        ligand_colors = c("T_specific" = "cyan2","Neutrophil_specific" = "purple", "Mononuclear-Phagocyte_specific" = "black", "general" = "green3"),
                                                        target_colors = receptor_colors,
                                                        celltype_order = c("T_specific",
                                                                           "Neutrophil_specific",
                                                                           "Mononuclear-Phagocyte_specific",
                                                                           "general"))

pdf(file = "~/20_015/nichenet_output2/Circos_Perio_Fibroblasts_Ligands-Receptors.pdf", width = 8, height = 8)
